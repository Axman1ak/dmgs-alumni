  ...
  return (
    <article className="grid items-center gap-7 lg:grid-cols-12 lg:gap-12">
      ...
      <Reveal delay={140} className={`lg:col-span-5 ${flip ? "lg:order-1" : ""}`}>
        <div className="font-display text-[40px] font-semibold leading-none text-gold-500/30">
          {String(index + 1).padStart(2, "0")}
        </div>
        <h3 className="mt-1.5 font-display text-[clamp(26px,3.2vw,36px)] font-medium leading-[1.05] text-emerald-900">
          <Link
            href={`/donations/projects/${project.slug}`}
        ...
        {project.goal > 0 && (
          <p className="mt-4 font-sans text-[12px] uppercase tracking-[0.16em] text-ink-muted">
            Goal{" "}
